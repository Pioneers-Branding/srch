(function ($) {
    ...
    $(document).on('click', '[data-slot-module]', function() {
     ...
     $(document).on('click', '[data-step-module]', function() {
        const offcanvas = document.querySelector($(this).data('step-target'))
        const eventName = $(this).data('step-event')
        const data = $(this).data('step-module')
        if(offcanvas) {
            const instance = checkOffcanvasInstance(offcanvas)
            createCustomEvent(eventName, {form_id: data})
            instance.show()
