    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.status && res.data) {
                console.log("Cart API Response:", res); // 🔍 Full API response
        ...
        error: function(error) {
            ...
            if (error.responseText) {
                console.log('Response Text:', error.responseText); // backend का पूरा error देखो
            }
            $('.cart-count1').text('0');
        }
...
function addProductElement(product) {
    ...
    <tr class="cart_item" id="product-${product.id}">
        <td class="product-remove">
            <a href="#" class="remove" onclick="removeProduct(${product.id})">×</a>
        </td>
        ...
        <td class="product-price" data-title="Price">
            <span class="Price-amount">
                <span class="Price-currencySymbol">₹</span>${product.default_price}
            </span>
        </td>
        ...
        <td class="product-subtotal" data-title="Total">
            <span class="Price-amount">
                <span class="Price-currencySymbol">₹</span>${product.default_price * product.quantity}
            </span>
        </td>
    </tr>
    `;
...
function updateCartTotal(totalAmount) {
    $('#cart-subtotal').text('₹' + totalAmount); // Ensure you have an element with the class 'cart-subtotal'
}
