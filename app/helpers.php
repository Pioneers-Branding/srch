                ...
                case 'manager':
                    if (isset($data['vendor_ids'])) {
                        foreach ($data['vendor_ids'] as $vendorId) {
                            $employee = \App\Models\User::find($vendorId);
...
function sendPushNotification($user, $title, $body, $image = null)
{
        $deviceToken = $user->device_token;
        $serverKey = !empty($user->server_key) ? $user->server_key : env('FIREBASE_SERVER_KEY');
        
        if (!empty($user->device_token) && !empty($user->server_key)) {
        
                $postArray = [
                    'title' => $title,
                    'body' => $body,
                    'sound' => 'tiwa_ringtone.mp3', 
                    // url('notification/nokia-original-ringtone-tkkb.mp3'),
                ];
