...
@section('content')
        ...
        {{--          </button>--}}
                ...
                <x-slot name="toolbar">
                    ...
                    @hasPermission('add_service')
                        <x-buttons.offcanvas target='#form-offcanvas' class="create_btn"  title="{{ __('messages.create') }} {{ __($module_title) }}">
                        {{ __('messages.create') }} {{ __('service.singular_title') }}</x-buttons.offcanvas>
                    @endhasPermission
                    <button class="btn btn-outline-primary btn-group d-none" data-bs-toggle="offcanvas"
    ...
    <div data-render="app">
        ...
        <step-add-form-offcanvas></step-add-form-offcanvas>
    </div>
...
@push('after-scripts')
    ...
    <script src="{{ asset('modules/service/script.js') }}"></script>
    ...
    <script type="text/javascript" defer>
        const columns = [{
            ...
            // { data: 'employee_count', name: 'employee_count',  title: "{{ __('Vendors') }}", orderable: true, searchable: false,  },
            @endif
            {
                data: 'status',
                name: 'status',
        ...
        // function childCategorytag(data , id=""){

        //     var html = '<div id="child_category_idsd" class="mb-2"><label class="form-label" for="child_category_id">Child Category</label> <select class="form-control" name="child_category_id" id="child_category_id"><option value=""></option>';
        ...
        //         $('#feature_image').closest('.form-group').before(html);
        //     }
        // }
        // var sub_category_id='';
        // var category_id='';
        ...
        //     var sub_text = $(this).closest('.multiselect').find('.multiselect-single-label-text').text();
        //     var sub_length = $(this).closest('.multiselect').find('#sub_category_id').length;

        //     var cat_text = $(this).closest('.multiselect').find('.multiselect-single-label-text').text();
        //     var cat_length = $(this).closest('.multiselect').find('#category_id').length;


        //     if (cat_length != 0 ) {
                
        //         $.ajax({
        //             url: "{{ route('backend.services.get_category_id') }}",
        //             method: "GET",
        //             data: {
        //                 text: cat_text
        //             },
        ...
        //         $.ajax({
        //             url: "{{ route('backend.services.get_child_category') }}",
        //             method: "GET",
        //             data: {
        //                 sub_text: sub_text
        //             },
        ...
        //                 sub_category_id = data.sub_id;

        //                 $('#child_category_idsd').remove();

        //                 childCategorytag(data.child);
        //             },
        //             error: function (xhr, status, error) {
        //                 console.error("Error:", status, error);
        //             }
        ...
        //     // document.getElementById('form-offcanvas').classList.toggle('show');
        //     // document.getElementsByClassName('offcanvas-backdrop')[0].classList.toggle('show');

        //     var secondButton = $(".offcanvas-footer button:eq(1)");
        //     secondButton.trigger("click");
        // }
       
        //  var serviceForm = $('#form-offcanvas').parent();
        ...
        //     var edit_id = $(this).data('crud-id');
        //     service_id = edit_id;

        //     $('#child_category_idsd').remove();

        //             $.ajax({
        //                 url: `{{ url('app/') }}/services/${edit_id}/edit`,
        //                 method: "GET",
        //                 dataType: "json",
        //                 success: function (data) {
        ...
        //                     console.log(data.data.sub_category_id);

        //                     if (data.length !== 0) {
        //                         getChildCategory(sub_category_id , data.data.child_category_id);
        //                     }
        //                 },
        //                 error: function (xhr, status, error) {
        //                     console.error("Error:", status, error);
        ...
        //             function getChildCategory(subCategoryId , childCategoryId) {
        //                 $.ajax({
        //                     url: "{{ route('backend.services.get_child_category') }}",
        //                     method: "GET",
        //                     data: {
        ...
        //                     dataType: "json",
        //                     success: function (data) {
        //                         childCategorytag(data.child , childCategoryId);
        //                     },
        //                     error: function (xhr, status, error) {
        //                         console.error("Error:", status, error);
        //                     }
        ...
        //     ajax_url = "{{ route('backend.services.store') }}";
        //     service_id= '';
        ...
        //     $('.form-group').find('span.text-danger:last').html('');

        //     var formId = $('#form-offcanvas');
        //     var name = formId.find('#Name').val();
        //     var description = formId.find('#description').val();
        //     var duration_min = formId.find('[name=duration_min]').find('input').val();
        //     var default_price = formId.find('[name=price]').find('input').val();
        //     var status = formId.find('#category-status').val();
        //     var is_feature = formId.find('#is-feature').val();
        //     var child_category_id = formId.find('#child_category_id').val();


        //     if (name=='') {
        //         formId.find('#Name').next('span').html('Name is a required field');
        //         return false;
        //     }

        //     if (duration_min=='') {
        //         formId.find('[name=duration_min]').find('input').next('span').html('Service Duration ( Mins ) is a required field');
        //         return false;
        //     }

        //     if (default_price=='') {
        //         formId.find('[name=price]').find('input').next('span').html('Default Price is a required field');
        //         return false;
        //     }
    
        //     if (typeof category_id === 'undefined' || category_id === undefined) {
        //         $('#category_id').parent().parent().parent().find('span.text-danger:last').html('Category is a required field');
        //         return false;
        //     }

           

        //     var feature_image = $('#feature_image').prop('files')[0];
        ...
        //     formData.append('duration_min', duration_min);
        ...
        //             type: 'POST',  // Corrected: Use 'PATCH' instead of 'patch'
        //             headers: {
