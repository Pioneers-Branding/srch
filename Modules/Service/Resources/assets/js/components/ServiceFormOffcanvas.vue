    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="form-offcanvas" aria-labelledby="form-offcanvasLabel">
      ...
      <div class="offcanvas-body">
        ...
        <div class="form-group" v-if="subCategories.options.length > 0">
          ...
          <Multiselect v-model="sub_category_id" :value="sub_category_id" v-bind="subCategories" id="sub_category_id" @select="changeSubCategory"></Multiselect>
        ...
        <!-- <div class="form-group" v-if="childCategories.options.length > 0">
          <label class="form-label" for="child_category_id">{{ $t('Child Category') }} </label>
          <Multiselect v-model="child_category_id" :value="child_category_id" v-bind="childCategories" id="child_category_id"></Multiselect>
          <span v-if="errorMessages['child_category_id']">
            <ul class="text-danger">
              <li v-for="err in errorMessages['child_category_id']" :key="err">{{ err }}</li>
            </ul>
          </span>
          <span class="text-danger">{{ errors.child_category_id }}</span>
        </div> -->
        ...
        <div class="form-group">
          <div class="d-flex justify-content-between align-items-center">
            <label class="form-label" for="is-feature">{{ $t('Is Featured') }}</label>
            <div class="form-check form-switch">
              <input class="form-check-input" :value="is_feature" :checked="is_feature" name="is_feature" id="is-feature" type="checkbox" v-model="is_feature" />
            </div>
          </div>
        </div>
...
import { EDIT_URL, STORE_URL, UPDATE_URL, CATEGORY_LIST , CHILD_CATEGORY_LIST } from '../constant/service'
    ...
    getRequest({ url: EDIT_URL, id: currentId.value }).then((res) => {
      if (res.status) {
        ...
        if (res.data.sub_category_id.value != 0) {
          ...
          changeSubCategory(res.data.sub_category_id);
        }
        if (res.data.child_category_id.value != 0) {
            child_category_id.value = res.data.child_category_id
        }
      }
    })
  } else {
...
const validationSchema = yup.object({
  ...
  duration_min: yup.string().required('Service Duration ( Mins ) is a required field'),
...
const { value: child_category_id } = useField('child_category_id') 
...
const changeSubCategory = (value) => {
  child_category_id.value = null
  listingRequest({ url: CHILD_CATEGORY_LIST, data: { id: value } }).then((res) => (childCategories.value.options = buildMultiSelectObject(res, { value: 'id', label: 'name' })))
}
