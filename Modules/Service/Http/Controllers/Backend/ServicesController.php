...
use Modules\Service\Models\ServiceStep;
...
class ServicesController extends Controller
{
   ...
   public function getChildCategoryList(Request $request)
    {
        $childCategory = ChildCategory::where('sub_id', $request->sub_id)->get();
    ...
    public function getChildCategory(Request $request)
    {
     ...
     if (!empty($request->get_by_sub_id)) 
     {
        $sub_id = $request->sub_id;
     }else
     {
        $sub  =  Category::whereNotNull('parent_id')->where('name' , $request->sub_text)->first();
        $sub_id = isset($sub->id) ? $sub->id : '' ;
     }
    
      $data['child'] = ChildCategory::where('sub_id' , $sub_id)->get();
      $data['sub_id'] = $sub_id;
    ...
    public function getSubCategoryId(Request $request)
    {
      ...
      $category  =  Category::whereNull('parent_id')->where('name' , $request->text)->first();
      
      $data['cat_id'] = isset($category->id) ? $category->id : '' ;
    ...
    public function store(ServiceRequest $request)
    {
        ...
        if(!empty($request->id))
        {
            $query = Service::findOrFail($request->id);
    ...
    {
        $service = Service::findOrFail($id);
        $data = ServiceStep::where('service_id', $id)->get();

        return response()->json(['data' => $data, 'service' => $service, 'status' => true]);
    }

    public function uploadSteps(Request $request, $id)
    {
        $stepArray = collect($request->steps, true);
    
        $images = ServiceStep::where('service_id', $id)->whereNotIn('id', $stepArray->pluck('id'))->get();
    
        foreach ($images as $key => $value) {
            if ($value->exists()) {
                $value->clearMediaCollection('step_image');
                $value->delete();
            }
        }
        ...
        foreach ($stepArray as $stepData) {
            if ($stepData['id'] == 'null') {
                $serviceStep = ServiceStep::create([
                    'service_id' => $id,
                    'step' => isset($stepData['step']) ? $stepData['step'] : '',
                    'description' => isset($stepData['description']) ? $stepData['description'] : '',
                ]);
            } else {
                $serviceStep = ServiceStep::find($stepData['id']);
                if ($serviceStep) {
                    $serviceStep->update([
                        'step' => isset($stepData['step']) ? $stepData['step'] : '',
                        'description' => isset($stepData['description']) ? $stepData['description'] : '',
                    ...
                    return response()->json(['message' => __('Service Step not found'), 'status' => false]);
                }
            }
    
            if (isset($stepData['file'])) {
                
                if ($stepData['file'] !='null') {
                    $serviceStep->clearMediaCollection('step_image'); 
                    $serviceStep->addMedia($stepData['file'])->toMediaCollection('step_image');
                    $serviceStep->full_url = $serviceStep->getFirstMediaUrl('step_image');
                }
            }
            $serviceStep->save();
        }
    
        return response()->json(['message' => __('Service Steps Updated Successfully'), 'status' => true]);
    }
}
