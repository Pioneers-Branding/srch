...
class ServiceController extends Controller
{
    ...
    public function serviceList(Request $request)
    {
        ...
        $child_category_id = $request->childcategory_id;
        
        $perPage = $request->input('per_page', 10);
        $branchId = $request->input('branch_id');

        $services = Service::with(['media', 'branches', 'employee' , 'child_category']);
        ...
        if (!empty($child_category_id)) {
            ...
            $services->where('child_category_id', $child_category_id);
        }
        
        if ($request->has('featured')) {
        
            $services->where('is_feature', $request->featured);
        }
    ...
    {
        $services = Service::where('status', 1)->with(['category', 'sub_category', 'child_category' ,'branches']);
        ...
        if ($request->has('child_category_id')) {
            $services->where('child_category_id', $request->child_category_id);
        }
    ...
    public function ServiceSteps(Request $request)
    {
        ...
        if ($serviceId) {
            ...
            $data = ServiceStep::where('service_id', $serviceId)->get();

            $steps = ['step' => $data, 'service' => $service];

            return response()->json([
                'status' => true,
                'data' => $steps,
                'message' => __('service stpes list'),
            ], 200);
        }else {
            ...
            return response()->json([
                'status' => false,
                'data' => null,
                'message' => __('service_id is required'),
            ], 200);
        }
    }
}
