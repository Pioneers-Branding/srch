...
class BookingsController extends Controller
{
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $query = Booking::query()->branch()->with('user', 'services', 'mainServices' , 'commissionBooking' , 'bookingTransaction');
        ...
        $booking_status = Constant::getAllConstant()->where('type', 'BOOKING_STATUS')->where('name', '!=', 'completed')->where('status', '1');
    ...
    public function BookingPaymentStatusUpdate(Request $request)
    {
        $commissionEarning = CommissionEarning::find($request->id);
        
        if ($commissionEarning) {
            
            if ($request->vendor_payment_status) {
                $commissionEarning->vendor_payment_status = $request->vendor_payment_status;
            }
          ...
          return response()->json(['status' => false, 'message' => 'CommissionEarning not found.']);
        }
    }
    
    public function getBookingVendorProduct($booking_id)
    {
    
         $bookingVendorProduct = BookingVendorProduct::where('booking_id' , $booking_id)->with('product')->latest()->get();
         
        
         foreach ($bookingVendorProduct as $list) {
             $list->product_main_image = $list->product->feature_image ?? ''; 
         }
         
         return $bookingVendorProduct;
    }
}
