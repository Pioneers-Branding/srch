      ...
      <div class="offcanvas-body">
        <div class="row">
          <div class="col-12">
            ...
            <div class="form-group">
              <div class="text-center">
                <img :src="BannerViewer || props.defaultImage" alt="banner-image" class="img-fluid mb-2 avatar-140 avatar-rounded" />
              </div>
              <label class="form-label" for="feature_image">{{$t('Banner Image')}}</label>
              <input type="file" class="form-control" id="banner" @change="bannerUpload" accept=".jpeg, .jpg, .png, .gif" />
              <span v-if="errorMessages['banner']">
                <ul class="text-danger">
                  <li v-for="err in errorMessages['banner']" :key="err">{{ err }}</li>
                </ul>
              </span>
              <span class="text-danger">{{ errors.banner }}</span>
            </div>
...
const BannerViewer = ref(null)
const bannerUpload = async (e) => {
  let file = e.target.files[0]
  await readFile(file, (fileB64) => {
    BannerViewer.value = fileB64
  })
  banner.value = file
}
...
const setFormData = (data) => {
  ...
  BannerViewer.value = data.banner
