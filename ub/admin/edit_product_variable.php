...
$post_tbl = 'product';
   ...
   <script>
       ...
       window.location.href='edit_product_variable?product=<?= $id ?>';
   </script>
   <?php 
   }
   ?>
        ...
        if ($query->num_rows > 0)
        {
            ...
            while ($rowa = $query->fetch_assoc())
            {
                 echo '<option value="' . $rowa['cat_pk_id'] . '"'; foreach ($categories as $category) { if($category == $rowa['cat_pk_id'])echo 'selected';} echo ' >' . $sub_mark . $rowa['category'] . '</option>'; 
                    ...
                    <?php echo $core_control_oj->displaySub($row['product_cat'],$row['product_sub_cat']);?>
                    </select>
                  </div>
                  </div>
                     <div class="row col-md-12">
                ...
                <div class="row col-md-12">
                  ...
                  <div class="form-group col-md-12">
                    <label for="exampleInputEmail1">Product Description(French)</label>
                      <textarea name="u_product_desc_fr" class="form-control" id="u_product_desc_fr" ><?=$row['product_desc_fr'];?></textarea>
                  </div>
                </div>
                <!-- /.card-body -->
