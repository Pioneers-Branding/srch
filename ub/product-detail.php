...
if(isset($row['product_pk_id']) && !empty($row['product_pk_id'])){
    ...
    $sizeDeatails = towquery("SELECT * FROM `tbl_size` WHERE size_status=1");
          ...
          //category
    //  $productcat = explode(",",$productfetch['product_cat']);
    $productCat=$productfetch['product_cat'];
    
    
    $firstb=substr($productCat, 1);
    $length=strlen($firstb);
    $Cat=substr($firstb, 0,$length-1);
    
    $categoryDeatails = towquery("SELECT * FROM `tbl_category` WHERE cat_pk_id in ($Cat) ");
    $catecoryname= towfetch($categoryDeatails);
...
?>
<nav aria-label="breadcrumb"z>
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="https://feminineafrica.com/"><?= Home ?></a></li>
    <!--<li class="breadcrumb-item"><a href="#">Library</a></li>-->
    <li class="breadcrumb-item"><a href="<?= base_url ?><?= $productfetch['cat_url'] ?>"><?php echo $catecoryname['category']; ?></a></li>
			...
			<div class="row">
				...
				<div class="col-lg-7 single-right-left simpleCart_shelfItem">
					...
					<p class="mb-3" style="font-family: 'Nunito Sans', sans-serif;">
						<span class="item_price"><?php if(isset($_SESSION['exchange_rate']))
                 { echo $_SESSION['exchange_sign']; }
                 else
                 {
                 echo "CFA";}?>
                 <?php if(isset($_SESSION['exchange_rate'])){
                     echo number_format( $_SESSION['exchange_rate']*($discount),2); } else {
                 echo $discount;}?></span>
					...
					<label style="font-weight: bold">Product Information</label>
					...
					<div class="occasion-cart" <?php if($productfetch['stock_quantity']<'1'){ ?> style="display: none;" <?php } ?>>
						<div class="snipcart-details top_brand_home_details item_add single-item hvr-outline-out">
							<form action="#" method="post">
							    ...
							    <input type="hidden"  value="<?php echo $pkid; ?>" name="product_pk" id="product_pk" class="form-control" />
							...
							<label style="display:none" ><?= brand ?></label>
							  ...
							  <label style="font-weight: bold"><?= color ?></label>
                                    ...
                                    <?php 
                                    $colorvariation = towquery("SELECT * FROM `tbl_product_variation`left join tbl_color on tbl_product_variation.product_color_id=tbl_color.color_id WHERE tbl_product_variation.product_id='$pkid' GROUP BY tbl_product_variation.product_color_id");
                                    //$colorfetch = towfetch($colorvariation);
                                   // print_r($colorfetch);
                                    while($colorfetch = mysqli_fetch_assoc($colorvariation)){ 
                                       // print_r($colorfetch);
                                        
                                     ?>
                                        <div style="display:flex; align-items: center">
                                            <p class="color color_name" id="color_box_<?= $colorfetch[$color_title]?>" style="background-color:<?= $colorfetch[$color_title_eng]?>" onclick="selectColor('<?= $colorfetch[$color_title]?>')" data-id="<?= $colorfetch[$color_id]?>">
                                                <span class="tooltiptext"><?= $colorfetch[$color_title]?></span>
                                            </p>
                                        </div>
                                    <?php }  ?>
                                    </div>   
							   ...
							   <?php if(!empty($colorName['color_name'])){ ?>
							    ...
							    <div style="display: flex; margin-right:7px" id="sizediv">
    		    ...
    		    <?php foreach ($related_products as $product) { ?>
    		        <a class="col-md-3 col-sm-12" href="<?= base_url ?><?=$product['product_url']?>">
    		            ...
    		            <div>
    		                ...
    		                <p class="related_product" style="font-weight: bold"><?php if(isset($_SESSION['exchange_rate']))
                 { echo $_SESSION['exchange_sign']; }
                 else
                 {
                 echo "CFA";}?>
                 <?php if(isset($_SESSION['exchange_rate'])){
                     echo  number_format($_SESSION['exchange_rate']*($product['discount']),2); } else {
                 echo $product['discount'];}?></p>
    		            </div>
    		        </a>
    		    <?php } ?>
		    </div>
...
<script>
    ...
    function selectColor(color){
        ...
        document.getElementById(colorSelector).style.boxShadow = "0 0 3px 1px #00699d";
         var product_id = document.getElementById('product_pk').value;

       //alert(product_id);
        
         $.ajax({ 
            url: 'getsize.php',
            data: { selectedcolor: color, product_id: product_id},
            type: 'POST'
        }).success(function(responseData) {
            console.log('Done: ', responseData);
            document.getElementById("sizediv").innerHTML= responseData;
        })
