...
// print_r($_GET['maxp']);
        ...
        if(!empty($_GET['maxp'])){
            ...
            for($ft=0;$ft<count($minarray);$ft++){
             $query = " SELECT `product_pk_id`,`discount` FROM `tbl_product` WHERE `brand_fk_id`=$cid AND `discount` >= ".$minarray[$ft]." AND `discount` <= ".$maxarray[$ft];   
...
//   print_r($_GET['pt']);
        if(!empty($_GET['pt'])){
            foreach($_GET['pt'] as $pt){
               $querypt = " SELECT `product_pk_id`,`discount` FROM `tbl_product` WHERE `brand_fk_id`=$cid AND `type_fk_id` = ".$pt;   
...
//   print_r($_GET['cl']);
        if(!empty($_GET['cl'])){
            foreach($_GET['cl'] as $clc){
               $querycl = " SELECT `product_pk_id`,`discount` FROM `tbl_product` WHERE `brand_fk_id`=$cid AND `color` = ".$clc;   
                // echo $querycl;
...
//   print_r($_GET['sz']);
        if(!empty($_GET['sz'])){
            foreach($_GET['sz'] as $szp){
               $querysz = " SELECT `product_pk_id`,`discount` FROM `tbl_product` WHERE `brand_fk_id`=$cid AND `size` like '%".$szp."%' ";   
                // echo $querysz;
...
//Create Url
if(!empty($_GET['st']) || !empty($_GET['pt']) || $_GET['maxp']){
$url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
$url=parse_url($url);
$clearURL = base_url.$cat['brand_url'];
...
<style>
              ...
              <div class="row">
                    <div class="col-md-2" style="margin-left:20px">
                        <form method="get" action="<?= base_url ?><?= $cat['brand_url']?>">
                           <ul>
                               <li class="dropdown">
                                    <a href="#" data-toggle="dropdown" class="dropdown-toggle-sort" style="color: #999;">
                                    <?= sort_by ?> <i class="fa fa-chevron-right" style="color: #999;"></i></b>
                                    </a>
                ...
                <form method="get" action="<?= base_url?>brand-filter" style="margin-left:160px;margin-top:-24px"> 
   ...
   <ul>
       <li class="dropdown">
            <a href="#" data-toggle="dropdown" class="dropdown-toggle-pdtype" style="color: #999;">
            <?= product_type ?> <i class="fa fa-chevron-right" style="color: #999;"></i></b>
            </a>
...
<div class="col-md-2">
  <div class="form-group">
    ...
    <ul>
       <li class="dropdown">
            <a href="#" data-toggle="dropdown" class="dropdown-toggle-color" style="color: #999;">
            <?= color ?> <i class="fa fa-chevron-right" style="color: #999;"></i></b>
            </a>
...
<!--        <input type="hidden" name="cid" value="<?= $cid ?>" readonly class="form-control" />-->
    <ul>
       <li class="dropdown">
            <a href="#" data-toggle="dropdown" class="dropdown-toggle-size" style="color: #999;">
...
<!--</form>     -->
                ...
                <div class="form-group" style="margin-top: -5px;">
                 <ul>
                   ...
                   <li class="dropdown">
                        <a href="#" data-toggle="dropdown" class="dropdown-toggle-pr" style="color: #999;">
                       <?= price_range ?> <i class="fa fa-chevron-right" style="color: #999;"></i></b>
                        </a>
            ...
            <div class="row">
                	<?php $uid = (isset($_SESSION['userId']))?$_SESSION['userId']:"";
                	 foreach($filterQueryId as $proId){
                	        $filterquery = towquery("SELECT * FROM `tbl_product` WHERE  `product_pk_id`=$proId AND  `brand_fk_id`=$cid");
