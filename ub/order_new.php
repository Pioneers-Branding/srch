...
if (isset($_POST['submit'])) {
    ...
    if (isset($email) && !empty($email) && !empty($last_name)) {
        echo "select * FROM `tbl_user` WHERE `user_email`='$email'";
        ...
        if ($checkUser->num_rows > 0) {
            ...
            $query = "UPDATE `tbl_user` SET `full_name`='$user_full_name',`first_name`='$first_name',`contact`='$phone',`last_name`='$last_name',`user_email`='$email',`address`='$userShippingAddress',`addressType`='$addressType',`landmark`='$landmark' WHERE `user_pk_id`='$newuser_user_pk_id'";
            $udid = towquery($query);
        } else {

            $query = "INSERT INTO `tbl_user`(`full_name`, `user_email`, `contact`,`user_role`,`user_status`,`user_reg_date`,`user_password`) VALUES ('$user_full_name','$email','$phone','user','1','" . date('Y-m-d H:i:s') . "','$pass')";
            $pass = md5($user_password);
            $newuser_user_pk_id = towquery2($query);
        }
    ...
    while (count($product_pk_id) > $i) {
        ...
        if (isset($address) && !empty($zipcode)) {
            ...
            $udid = towquery2("INSERT INTO `tbl_transaction`(`order_id`, `user_fk_id`, `transaction_status`, `transaction_mode`, `transaction_date`, `del_addr`, `transc_amount`, `leftAmount`, `tContact`, `tPincode`, `tName`, `tEmail`, `tState`, `tCity`, `t1Name`, `t1Email`, `t1Contact`, `t1State`, `t1City`, `t1Pincode`, `userShippingAddress1`, `trackingId`, `trackingLink`, `shiping_ch`,`shipingPrice`,`shipingDay`,`shipingTime`,`totalamt`,`currency_code`,`currency`,`addressType`,`userShippingLandmark`,`couponName`,`couponPrice`) 
                                          VALUES ('ORDER-" . $d . "','$newuser_user_pk_id',0,'COD','" . date('Y-m-d H:i:s') . "','$address',0,'$totalamt','$phone','$zipcode','$user_full_name','$email','','','$user_full_name','$email','$phone','','','$zipcode','$address','','','','$shipping','$shipingDay','$slotTime','$totalamt','$currency_code','$currency','$addressType','$landmark','$discouponName','$discouponPrice')");
        } else {
            $udid = towquery2("INSERT INTO `tbl_transaction`(`order_id`, `user_fk_id`, `transaction_status`, `transaction_mode`, `transaction_date`, `del_addr`, `transc_amount`, `leftAmount`, `tContact`, `tPincode`, `tName`, `tEmail`, `tState`, `tCity`, `t1Name`, `t1Email`, `t1Contact`, `t1State`, `t1City`, `t1Pincode`, `userShippingAddress1`, `trackingId`, `trackingLink`, `shiping_ch`,`shipingPrice`,`shipingDay`,`shipingTime`,`totalamt`,`currency_code`,`currency`,`addressType`,`userShippingLandmark`,`couponName`,`couponPrice`) 
        ...
        if ($checkProduct->num_rows > 0) {
            ...
            $color_id =    towquery("Select color_id FROM `tbl_color` WHERE `color_name`='$cartclr' ");
            $rowclr = towfetch($color_id);
            $clrid = $rowclr['color_id'];

            $size_id =    towquery("Select size_id FROM `tbl_size` WHERE `size_name`='$cartsize' ");
            $rowsize = towfetch($size_id);
            $sizeid = $rowsize['size_id'];

            $updvar = towquery("UPDATE `tbl_product_variation` SET `product_quantity`= product_quantity - $qty[$i] WHERE `product_id`='$product_pk_id[$i]' and `product_color_id`='$clrid' and `product_size`='$sizeid'");
        }
...
Nous espérons que votre achat vous apportera pleine satisfaction. Nous vous remercions d’avoir passé commande auprès de Feminine Africa et vous tiendrons informé de la progression de celle-ci au fur et à mesure. N’hésitez pas à revenir vers nous en cas de problème. <br>
...
Congratulations! 
<br><br>
We hope that you will enjoy your purchase at its fullest. We thank you for your order made with Feminine Africa and we will let you know, step by step how it is going. Let us know if you have any difficulty. 
...
Feminine Africa<br>
<a href='https://feminineafrica.ci/' target='_blank'>www.feminineafrica.ci</a>
<br><br>
<img src='https://feminineafrica.ci/images/mobile.png' style='width: 24px; height: 18px;'>+225 07 02 56 02 40
<br>
<img src='https://feminineafrica.ci/images/envelop.png' style='width: 18px; height: 16px;'>inquiry@feminineafrica.ci
<br>							 
<a href='https://www.instagram.com/feminineafrica/' target='_blank'>
<span>
<img src='https://feminineafrica.ci/images/instagram.png' style='width: 20px; height: 18px;'>
 @feminineafrica
</span>
</a>
<br>
<a href='https://www.youtube.com/channel/UCepcCf9SScPmSq0KGWxL8RQ' target='_blank'>
<span>
<img src='https://feminineafrica.ci/images/youtube.png' style='width: 20px; height: 18px;'>
Feminine Africa
</span>
</a>";

    $header = "From:inquiry@feminineafrica.ci \r\n";
    $header .= "MIME-Version: 1.0\r\n";
    $header .= "Content-type: text/html\r\n";
    $retval = mail($to, $subject, $message, $header);
    $url = 'https://feminineafrica.ci/thank-you/'; //.$d;
...
<div class="container">
  ...
  oninput="this.setCustomValidity('')">
                              ...
                              <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="first_name" name="first_name" required oninvalid="this.setCustomValidity('Prénom (obligatoire')"
  oninput="this.setCustomValidity('')" >
                              </div>
                              <div class="form-group">
                                <label for="last-name"><?= l_name ?></label>
                ...
                <div class="information-wrapper">                    
                    <div class="first-row">                    
                        <div class="controls form-group">
        					...
        					<input class="billing-address-name form-control" type="text" name="first_name" required value="<?= isset($us['first_name'])?$us['first_name']:'' ?>"   placeholder="<?= ft ?>" >
        					
        					<input class="billing-address-name form-control" type="text" name="last_name" required value="<?= isset($us['last_name'])?$us['last_name']:'' ?>"   placeholder="<?= lt ?>" >
        				...
        				<div class="w3_agileits_card_number_grids">
        					<div class="w3_agileits_card_number_grid_left form-group">
        						<div class="controls">
        							<input type="text" class="form-control" placeholder="<?= mobile ?>" name="user_contact" value="<?= isset($us['contact'])?$us['contact']:'' ?>"  required>
        						</div>
        					</div>
        				...
        				<div class="controls form-group">
        					<!--<input type="text" >-->
        					<textarea class="form-control" placeholder="<?= address ?>" name="userShippingAddress" required=""><?= isset($us['address'])?$us['address']:'' ?> </textarea>
        				</div>
        				<div class="controls form-group">
        					<input type="text" class="form-control" placeholder="<?= landmark ?>" value="<?= isset($us['landmark'])?$us['landmark']:'' ?>" name="landmark" required="">
        				</div>
        ...
        <div class="col-md-12 col-xs-12" style="border-bottom:#ccc solid 1px; padding:15px 0 10px 0;">
            <div class="col-md-12 col-xs-12">
                <div class="row">
                 ...
                 { $sign=$_SESSION['exchange_sign']; }
                 else
                 {
                 $sign="CFA";}?>
                 <?php if(isset($_SESSION['exchange_rate'])){
                     $dis= number_format($_SESSION['exchange_rate']*($cartf['discount']),2); 
                     $pr=number_format($cartf['product_price']*($cartf['discount']),2); 
                 } else {
                 $dis=$cartf['discount'];
                 $pr=$cartf['product_price'];
                 }?>
                    <?php $decode = json_decode($cartf['product_image']);
                    $subtotal +=$pr* $cartf['qty'];
                    $total +=$dis* $cartf['qty']?>
                        ...
                        <div class="col-md-12 col-xs-12 padding_0 mv-15 flex-row">
                            ...
                            <div class="col-md-8 col-xs-12 padding_0" style="padding-left: 15px">
                                ...
                                <p style="color:#000;font-family: 'Nunito Sans', sans-serif;"><?php $sign;?>
                 <?php if(isset($_SESSION['exchange_rate'])){
                     echo  number_format($_SESSION['exchange_rate']*($cartf['discount']),2); } else {
                 echo $cartf['discount'];}?></p>
                            </div>
                        </div>
                    ...
                    <div class="col-md-4 col-xs-4 border-top">
                        <p id="discount" style="font-size: 14px;color: #000;text-align: right;"><?= $sign; ?> <?= currency_price($subtotal) ?> </p>
                        <p style="font-size: 14px;color: #000;text-align: right;"><?= $sign; ?> <?= currency_price($total-$subtotal) ?> </p>
                    </div>
                </div>
            </div>
       ...
       <?/*  <div class="col-md-12 col-xs-12" style="border-bottom:#ccc solid 1px; padding:15px 0 10px 0;">
            <div class="col-md-12 col-xs-12">
                <div class="row">
					...
					<p style="font-size: 16px;font-family: 'Nunito Sans', sans-serif; color: #000; font-weight: bold;" >
            ...
            <div class="col-md-12 col-xs-12">
                <div class="row">
                    ...
                    <div class="col-md-4 col-xs-4">
                        ...
                        <span id="tot" style="font-family: 'Nunito Sans', sans-serif;"><?= $sign ?> <?= currency_price($total) ?></span>
