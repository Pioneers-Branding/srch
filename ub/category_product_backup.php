...
$start_from = ($page-1) * $limit;
   ...
   // echo "SELECT * FROM `tbl_category` WHERE `cat_pk_id`=$cid ";
...
$maxPrice =  getPriceRaneforProduct($cid,"DESC");
     ...
     $query="";
        ...
        $query = "SELECT * FROM `tbl_product` WHERE `product_cat` LIKE '%$cid%'  ORDER BY product_pk_id DESC LIMIT $start_from, $limit";
        if(!empty($_GET['pt'])){
    ...
    //  echo $query;
     ...
     $pro = towquery("SELECT * FROM `tbl_product` WHERE `product_cat` LIKE '%$cid%'  ORDER BY product_pk_id DESC LIMIT $start_from, $limit" );
    }
    $procount = towquery("SELECT * FROM `tbl_product` WHERE `product_cat` LIKE '%$cid%'  ORDER BY product_pk_id" );
    $rowcount=mysqli_num_rows($procount);
      //printf("Result .\n",$rowcount);
...
<!--</form>     -->
        ...
        <p style="text-align: center; font-size: 14px;font-family: 'Nunito Sans', sans-serif; color: #000;" ><?= $rowcount; ?> styles found </p>
        ...
        <div class="container-fluid rty1" style="margin-bottom: 30px; margin-top: 20px;">
            <div class="row">
              ...
              //  $query1 = "SELECT * FROM `tbl_product` WHERE `product_cat` LIKE '%$cid%' ORDER BY product_pk_id DESC LIMIT $start_from, $limit";
            ...
            <div class="col-6 col-sm-3 oolp" style="margin-bottom: 25px; ">
              ...
              <a href="<?= base_url ?><?=$prof['product_url']?>">
                ...
                <p class="txtcatt"><?=$prof[$product_tilte]?></p>
                 <p style=" text-align: center; color: black; margin-top: 6px; font-size: 15px; font-family: 'Nunito Sans', sans-serif;"><?php if(isset($_SESSION['exchange_rate']))
                 { echo $_SESSION['exchange_sign']; }
                 else
                 {
                 echo "CFA";}?>
                 <?php if(isset($_SESSION['exchange_rate'])){
                     echo number_format($_SESSION['exchange_rate']*($prof['discount']),2); } else {
                 echo $prof['discount'];}?></span> <span style="text-decoration-line: line-through;font-size: 13.5px; color:ccc; margin-left: 5px;"></span></p>
            </a>
            </div>
                <?php } ?>
	...
	<!-- //banner -->
<div class="row"> <div class="col-md-5"></div><div class="col-md-4">
...
<style>
   ...
   sans-serif; background: #000;
   color:white;
   box-shadow: none !important;
    }
    
     .page-link:hover, .page:hover , .currentpage .page-link
    {
...
<ul class="pagination">
	<?php if ($page > 1): ?>
	<li class="prev" ><a class='page-link' href="?page=<?php echo $page-1 ?>">Prev</a></li>
	<?php endif; ?>

	<?php if ($page > 3): ?>
	<li class="start"><a class='page-link' href="?page=1">1</a></li>
	<li class="dots">...</li>
	<?php endif; ?>

	<?php if ($page-2 > 0): ?><li class="page">
	    <a class='page-link' href="?page=<?php echo $page-2 ?>"><?php echo $page-2 ?></a></li><?php endif; ?>
	<?php if ($page-1 > 0): ?><li class="page">
	    <a class='page-link' href="?page=<?php echo $page-1 ?>"><?php echo $page-1 ?></a></li><?php endif; ?>

	<li class="currentpage"><a class='page-link' href="?page=<?php echo $page ?>"><?php echo $page ?></a></li>

	<?php if ($page+1 < $totalPages +1): ?><li class="page">
	    <a class='page-link' href="?page=<?php echo $page+1 ?>"><?php echo $page+1 ?></a></li><?php endif; ?>
	<?php if ($page+2 < $totalPages +1): ?>
	<li class="page"><a href="?page=<?php echo $page+2 ?>" class='page-link'><?php echo $page+2 ?></a></li><?php endif; ?>

	<?php if ($page < $totalPages -2): ?>
	<li class="dots">...</li>
	<li class="end"><a class='page-link' href="?page=<?php echo $totalPages ?>"><?php echo $totalPages ?></a></li>
	<?php endif; ?>

	<?php if ($page < $totalPages): ?>
	<li class="next"><a class='page-link' href="?page=<?php echo $page+1 ?>">Next</a></li>
	<?php endif; ?>
</ul>
<?php endif; ?>
</div>
